...
ahead <- c(7, 14, 21, 28) - 2 # 5 ahead minus 2 because we predict the current week
geo_type <- "state" 
...
# bone headed way. results in 1 row per epiweek, but each row only uses results
# in that epiweek (at least at time 0, lags are much stranger because we lag
# first, then aggregate)
sum_to_epiweek <- function(df) { 
  df %>% bind_cols(MMWRweek::MMWRweek(df$time_value)) %>%
    mutate(time_value = MMWRweek::MMWRweek2Date(MMWRyear, MMWRweek, 7)) %>%
    group_by(geo_value, time_value) %>%
    summarise(across(starts_with("value"), ~ mean(.x, na.rm = TRUE) * 7)) %>%
    ungroup()
}

# somewhat better way. Use rolling averages first, then dump out 1 week per
# epiweek. Still lagged first, so still possibly weird.
grab_epiweek <- function(df) { 
  df %>% 
    ...
    bind_cols(MMWRweek::MMWRweek(df$time_value)) %>%
    filter(MMWRday == 2) %>%  # we always forecast on Monday.
    select(!starts_with("MMWR")) %>%
    ungroup()
}
...
bonehead <- get_predictions(
  forecaster = quantgen_forecaster, 
  name_of_forecaster = "bonehead",
...
```{r produce-more-forecasts}
middling <- get_predictions(
  forecaster = quantgen_forecaster, 
  name_of_forecaster = "middling",
  signals = tibble::tibble(
                      data_source = data_source, 
                      signal = data_signals,
                      start_day = list(start_day_quantgen)),
...
```{r produce-even-more-forecasts}
lots <- get_predictions(
  forecaster = quantgen_forecaster, 
  name_of_forecaster = "lots_of_data",
  signals = tibble::tibble(
                      data_source = data_source, 
                      signal = data_signals,
                      start_day = list(start_day_quantgen)),
...
qar_dc <- bind_rows(bonehead, middling, lots) %>% 
...
pd <- evalcast:::setup_plot_trajectory(qar_dc, intervals = 0.8,
                                       geo_type = "state", 
