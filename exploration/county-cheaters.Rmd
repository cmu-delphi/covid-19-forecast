...
zz <- zyzzyva::get_forecasters(n_locations = 200, weeks_back = 4)$zyzzyva_covidcast$forecaster

signals_cheater <- tibble(
...
aheads_cheater  <- 1:4
cheater  <- vector("list", length = length(aheads))

for (a in aheads_cheater) {
    cat(sprintf("Ahead = %d\n", ahead))
    cheater[[a]] <- evalcast::get_predictions(
      forecaster = zz,
      name_of_forecaster = "zyzzyva-cheater",
      signals = signals_cheater,
