...
remotes::install_github("cmu-delphi/covid-19-forecast", ref = "develop",
                         subdir = "utilities/zookeeper", upgrade = "never")
remotes::install_github("cmu-delphi/covid-19-forecast", ref = "develop",
                         subdir = "forecasters/aardvark", upgrade = "never")
```
...
start_day_mob <- function(forecast_date) {
  return(as.Date(forecast_date) - max(ahead) - n - max(unlist(lags)) + 1)
}
...
cases. This is at the state level. The mobility model has a 28 day lag from
the forecast date (should probably be from the target date)
...
```{r predictions-cheater}
sigs_cheater <- dplyr::tibble(
  data_source = "jhu-csse",
  signal = c("deaths_incidence_num", "confirmed_incidence_num"),
  start_day = "2020-03-07")
ahead_cheater <- 1:4
cheater  <- vector("list", length = length(ahead_cheater))

for (a in ahead_cheater) {
  aardvark_forecaster <- aardvark::get_forecasters(
    signals = sigs_cheater, ahead = a)[[1]]$forecaster
  cheater[[a]] <- get_predictions(forecaster = aardvark_forecaster,
                                  name_of_forecaster = "aardvark-cheater",
                                  signals = sigs_cheater,
                                  forecast_dates = forecast_dates,
...
cheater  <- dplyr::bind_rows(cheater)
```
