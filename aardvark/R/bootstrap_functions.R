...
make_by_location_gaussian_bootstrap_weekly <- function(ave, bandwidth){
  ...
  by_location_gaussian_bootstrap_weekly <- function(B, df_point_preds,
    ...
    point_preds <- df_point_preds %>%
      ...
      select(location, location_name, reference_date, preds)
    ...
    point_preds <- point_preds %>%
      ...
      mutate(reference_date = NA) %>%
      ungroup()

    # (2) Get conditional standard deviation estimates, on weekly scale
    df_resids <-  df_point_preds %>%
      select(location,location_name,reference_date, original_value, preds) %>% # we don't use strata for a by location bootstrap.
      filter(!is.na(original_value)) %>%
      mutate(resids = original_value - preds,
             weights = tricube( as.numeric(forecast_date - reference_date) / bandwidth)) %>%
    ...
    bootstrap_preds <- bind_cols(df_distribution,replicates) %>%
      select(location,location_name,reference_date,starts_with("replicate_"))
