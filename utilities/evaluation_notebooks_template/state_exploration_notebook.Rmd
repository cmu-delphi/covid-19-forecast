...
for (i in seq_along(forecast_dates)) {
  ...
  state_predictions2 <- bind_rows(state_predictions2, current_predictions)
}
```
...
for (i in seq_along(forecast_dates)) {
  ...
  state_forecaster_args$signals_to_normalize <- TRUE
  state_forecaster_args$transform <- function(x) log(x + 1)
  state_forecaster_args$inv_trans <- function(x) exp(x) - 1
  ...
  state_predictions3 <- bind_rows(state_predictions3, current_predictions)
}
```
...
state_predictions <- bind_rows(state_predictions, 
                               state_predictions2, 
                               state_predictions3)
