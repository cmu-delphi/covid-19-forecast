...
featurize_then_log <- function(df) {
  animalia::make_county_7dav_featurizer()(df) %>% 
    take_logs()
}
...
for (i in seq_along(forecast_dates)) {
  ...
  # additional tweaks
  county_forecaster_name <- "log"
  county_forecaster_args$featurize <- featurize_then_log
  county_forecaster_args$transform <- function(x) x  # log done in featurize step
  county_forecaster_args$inv_trans <- function(x) exp(x)
...
Get predictions for competitor 3 (production with population scaling for all
...
county_predictions <- bind_rows(prod_predictions, 
                                comp_predictions1, 
                                comp_predictions2,
                                comp_predictions3)
