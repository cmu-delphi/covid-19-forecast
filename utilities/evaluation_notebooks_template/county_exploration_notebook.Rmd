---
title: "Exploration (vs. Counties Production Forecaster)"
author: "Kenneth Tay"
date: "4/28/2021"
output: 
  html_document:
...
counties production forecaster. Often, the new forecaster is simply a tweak of
...
forecast_dates <- our_pred_dates[(n_dates - 7):(n_dates - 4)]
```
...
for (i in seq_along(forecast_dates)) {
  ...
  # additional tweaks
  county_forecaster_name <- "popn"
  county_forecaster_args$signals_to_normalize <- TRUE
  ...
  county_predictions2 <- bind_rows(county_predictions2, current_predictions)
}
```

Get predictions for competitor 2 (production with population scaling for all
signals, and log transform for all signals):

```{r competitor-predictions-2, results="hide"}
county_predictions3 <- NULL
for (i in seq_along(forecast_dates)) {
  # set parameters correctly for each forecast date
  Sys.setenv(FORECAST_DATE = as.character(forecast_dates[i]))
  source("../production_params.R")
  ...
  # additional tweaks
  county_forecaster_name <- "popn + log"
  county_forecaster_args$signals_to_normalize <- TRUE
  county_forecaster_args$transform <- function(x) log(x + 1)
  county_forecaster_args$inv_trans <- function(x) exp(x) - 1
  ...
  county_predictions3 <- bind_rows(county_predictions3, current_predictions)
}
```
...
county_predictions <- bind_rows(county_predictions, 
                               county_predictions2, 
                               county_predictions3)
