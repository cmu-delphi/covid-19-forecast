...
#' @export
#' @examples
#' special_flags <- special_state_params(c('wa','oh'), 
...
special_state_params <- function(
  geo_value = character(),
  signal = character(),
  time_value = character(),
  max_lag = numeric()) {
  # Make sure pairs matched for each state
  stopifnot(length(geo_value) == length(signal),length(signal) == length(time_value),
            length(time_value) == length(max_lag))
  
  # Return a tibble of zero row by default
  tibble::tibble(
    geo_value = geo_value,
    signal = signal,
...
#' (2) This function makes corrections on special flagging columns
...
make_special_correction <- function(df,special_flags){
  for (i in 1:nrow(special_flags)){
    ...
    df <- df %>% mutate(corrected = corrections_multinom_roll(
      ...
      special_flag = (.data[[varname]] == TRUE) 
    )
  }
  df$special_flag <- FALSE
  for (i in 1:nrow(df)){
    loc_of_corrected <- match('corrected', names(df))
    if (rowSums(df[i,(loc_of_corrected-nrow(special_flags)):(loc_of_corrected-1)]) > 0L) {
      df$special_flag[i] = TRUE 
    }
  }
  return(df)
}
...
aardvark_state_corrections_single_signal <- function(x, params, special_flags) {
  ...
  # Check if there is special correction to make
  if (nrow(special_flags) != 0) {
    
    x <- make_manual_flags(x, special_flags)
    
    # Correction on Rhode Island 
    x <- x %>% mutate(corrected = corrections_multinom_roll(
