...
library(DT)
library(ggforce)
knitr::opts_chunk$set(warning = FALSE, message=FALSE, dev="svg")
...
county_filtered <- county_filtered %>% group_by(geo_value)  %>% mutate(
  ...
  #                  & lead(value,n=3L)!=0)
  flag = flag & (time_value < ymd("2020-10-20") | value < -size_cut)
  )
...
```{r show-corrections, fig.height = 80, fig.width = 20, dev="svg"}
...
```{r}
sum_check = corrected_counties %>% filter(state != "RI") %>%
...
update_corrections("data_corrections.sqlite", "county", tosave)
```