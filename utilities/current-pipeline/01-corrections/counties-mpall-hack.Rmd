...
```{r grab-external, eval=min(check_recent$mr) <= "2021-01-31", cache=params$cache_data}
# shouldn't run if we have data after February 1
library(readr)
library(stringr)
process_usafacts <- function(raw) {
  raw %>%
    mutate(geo_value = str_pad(countyFIPS, 5, "left", 0)) %>% 
    filter(geo_value != "00000") %>%
    pivot_longer(cols = contains("-"), names_to = "time_value") %>%
    mutate(time_value = lubridate::ymd(time_value)) %>% 
    select(geo_value, time_value, value) %>%
    group_by(geo_value) %>% 
    mutate(value = value - lag(value, order_by = time_value)) %>% 
    ungroup() %>%
    mutate(data_source = "usa-facts",
           signal = "confirmed_incidence_num",
           issue = max(time_value),
           lag = 0L,
           stderr = NA,
           sample_size = NA) %>%
    filter(geo_value %in% !!todump, time_value > "2021-01-31")
}
county_cases <- read_csv(
  "https://static.usafacts.org/public/data/covid-19/covid_confirmed_usafacts.csv", 
  col_types = cols(countyFIPS = col_character()))
county_cases <- process_usafacts(county_cases)
county_filtered <- bind_rows(county_filtered, county_cases) %>%
...
county_filtered <- county_filtered %>% 
  left_join(df,by = c("data_source","signal","geo_value","time_value",
                      "issue","lag","stderr","sample_size","dt","value"))
...
tosave <- corrected_counties %>% 
  mutate(output = abs(corrected - value) > 0 | time_value > "2021-01-31") %>%
...
```{r impute-others, eval=all(check_recent$mr <= "2021-01-31"), cache=params$cache_data}
others <- suppressMessages(
  covidcast_signals(
    data_source = c("fb-survey", "indicator-combination"),
    signal = c("smoothed_hh_cmnty_cli", "nmf_day_doc_fbc_fbs_ght"),
    geo_type = "county", 
    start_day = "2021-01-15")) %>%
  aggregate_signals(format = "long") %>%
  filter(geo_value %in% !!todump, !is.na(data_source), !is.na(signal))
others <- others %>% 
  group_by(data_source, signal, geo_value) %>%
  filter(time_value > max(time_value) - 7) # most recent week
made_up <- others %>%
  mutate(time_value = time_value + 7) %>%
  # bind_rows(others) %>%
  ungroup() %>%
  arrange(time_value) %>%
  select(data_source, signal, geo_value, time_value, value) %>%
  mutate(corrected = value) %>%
  filter(time_value < Sys.Date()) %>%
  transmute(
    location = as.character(geo_value),
    location_name = as.character(NA),
    reference_date = as.Date(time_value), 
...
tosave <- bind_rows(tosave, made_up) %>% 
  as_tibble() %>%
  arrange(variable_name, location, reference_date)
```
