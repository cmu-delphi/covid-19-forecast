...
```{r show-corrections, fig.height = 30, fig.width = 10, dev="svg"}
corrected_states %>% group_by(geo_value) %>% 
  #filter(any(flag | flag_bad_RI | geo_value == "nc")) %>% 
...
tosave <- corrected_states %>% 
  ...
  transmute(
    ...
    variable_name = "usa-facts_deaths_incidence_num",
...
update_corrections("data_corrections.sqlite", "state", tosave)
```
